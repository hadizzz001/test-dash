...
export async function GET(req) {
  try {
    const categories = await prisma.offer1.findMany();
...
export async function POST(req) {
  try {
    ...
    const category = await prisma.offer1.create({ data: {  text,img } });
...
export async function PATCH(req) {
  ...
  try {
    ...
    const updatedCategory = await prisma.offer1.update({
      where: { id },
      data: {  text ,img},
    });
...
export async function DELETE(req) {
  try {
    ...
    const deletedCategory = await prisma.offer1.delete({ where: { id } });
